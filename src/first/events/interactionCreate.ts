import { ActionRowBuilder, ApplicationCommandType, type CacheType, type ColorResolvable, EmbedBuilder, type Interaction, StringSelectMenuBuilder } from 'discord.js'
...
export default class InteractionCreateEvent extends BotEvent {
  ...
  async execute(int: Interaction<CacheType>, client: FirstClientData) {
    const { emoji, owners, serverId } = client.data
  
    if(int.isChatInputCommand()){
      const { commandName, guildId } = int
  
      const publicCommand = client.slashCommands.get(commandName)
      client.data.usedCommands++
      if(publicCommand) return publicCommand.execute(int, client)
    ...
    if(int.isContextMenuCommand()){
      ...
      if(commandType == ApplicationCommandType.User){
        const publicCommand = client.contextCommands.get(commandName)
        client.data.usedCommands++
        if(publicCommand) return publicCommand.execute(int, client)
      }
    }
