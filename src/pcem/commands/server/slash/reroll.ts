...
export const rerollSlashCommand = async (int: ChatInputCommandInteraction<CacheType>) => {
  ...
  const dataSor = await rafflesModel.findById(serverId), arraySo = dataSor?.raffles, messageId = options.getString("id", true)
  ...
  const raffle = arraySo?.find(f=> f.id==messageId), channel = guild?.channels.cache.get(raffle?.channelId || ''), message = (channel?.type == ChannelType.GuildText || channel?.type == ChannelType.GuildAnnouncement) ? channel.messages.cache.get(messageId) : undefined
  const participants = raffle?.participants.filter(f=> guild?.members.cache.has(f))
  ...
  if(raffle && participants){
    for(let r=0; r<bueltas; r++){
      ...
      if(raffle?.winners > ganadoresFinal.length && !ganadoresFinal.some(s=>s==miembroRandom)){
  ...
  .setDescription(`Se ha vuelto a seleccionar ${raffle?.winners==1 ? "el ganador ": "los ganadores"} del sorteo que esta en ${channel?.id == int.channelId ? `este canal`: `el canal <#${channel?.id}>`}.`)
  .setColor("#00ff00")
