...
export const informacionSlashCommand = async (int: ChatInputCommandInteraction<CacheType>) => {
  ...
  if(subCommand == "miembro"){
    const dataAli = await alliancesModel.findById(botDB.serverId), userOption = options.getUser('usuario')
    ...
    if(member){
      ...
      if(member.id == user?.id){
        ...
        if(isStaff && miembroAli) memberInfoEb.data.fields?.push({name: `🤝 **Alianzas:**`, value: `Has creado ${(miembroAli?.amount || 0) <=1 ? `**${miembroAli?.amount}** alianza.`: `**${miembroAli?.amount}** alianzas.`}`})
      }else{
        memberInfoEb
        ...
        if(isStaff && miembroAli) memberInfoEb.data.fields?.push({name: `🤝 **Alianzas:**`, value: `Ha creado ${(miembroAli?.amount || 0) <=1 ? `**${miembroAli?.amount}** alianza.`: `**${miembroAli?.amount}** alianzas.`}`})
      }
