...
export const finalizarSlashCommand = async (int: ChatInputCommandInteraction<CacheType>, client: Client) => {
  ...
  if(subCommand == "encuesta"){
    const dataEnc = await surveysModel.findById(serverId), arrayEn = dataEnc?.surveys, messageId = int.options.getString('id', true)
    ...
    if(setSlashErrors(int, [
      ...
      [
        Boolean(!arrayEn?.filter(f=> f.active).some(s=>s.id == messageId)),
    ...
    const encuesta = arrayEn?.find(f=> f.id == messageId), canal = client.channels.cache.get(encuesta?.channelId || ''), message = canal?.type == ChannelType.GuildText ? canal.messages.cache.get(messageId) : undefined
    if(message){
      let opcionesOrdenadas = encuesta?.options.sort((a,b)=> b.votes - a.votes), totalVotos = 0, bueltas = 1, tabla = []
      opcionesOrdenadas?.forEach(m=> totalVotos += m.votes)
      ...
      if(encuesta?.options.filter(f=> f.votes>0).length==0) return setSlashError(int, `No puedes finalizar la encuesta ya que nadie ha participado en ella.`)
  ...
  if(subCommand == "sorteo"){
    const dataSor = await rafflesModel.findById(serverId), arraySo = dataSor?.raffles, messageId = int.options.getString('id', true)
    ...
    if(setSlashErrors(int, [
      ...
      [
        Boolean(!arraySo?.filter(f=> f.active).some(s=>s.id == messageId)),
    ...
    const sorteo = arraySo?.find(f=>f.id == messageId), canal = client.channels.cache.get(sorteo?.channelId || ''), message = canal?.type == ChannelType.GuildText ? canal.messages.cache.get(messageId) : undefined
    if(message && sorteo){
      const miembros = sorteo?.participants.filter(f=> guild?.members.cache.has(f))
