...
export const crearSlashCommand = async (int: ChatInputCommandInteraction<CacheType>, client: Client) => {
  ...
  if(subCommand == "alianza"){
    const dataAli = await alliancesModel.findById(serverId), arrayMi = dataAli?.members, arraySv = dataAli?.servers
      ...
      if(m){
        ...
        client.fetchInvite(plantilla.split(/ +/g).find(f=> ["discord.com/invite/", "discord.gg/"].some(s=> f.includes(s))) || '').then(async invite=> {
          if(arraySv?.some(s=> s.id == invite.guild?.id)){
            ...
            if(servidor){
              if(servidor.time >= Math.floor(Date.now()-ms("7d"))) return int.editReply({embeds: [
                createEmbedMessage(
                  `${emoji.negative} Error`, 
                  `Ya se ha echo alianza con ese servidor y esa alianza se ha echo **<t:${Math.floor((servidor?.time || 0)/1000)}:R>** así que no se puede renovar por ahora.`, 
                  color.negative
                )
              ]})
  ...
  if(subCommand == "encuesta"){
    const dataEnc = await surveysModel.findById(serverId), arrayEn = dataEnc?.surveys
    ...
    let cantidadOpciones = 0, posicion = 0, opciones: {emoji: string, option: string, votes: number}[] = []
    ...
    .addFields(
      {name: `🧩 **Opciones**`, value:`${opciones.map(m=> `${m.emoji} ${m.option}`).join("\n")}`},
    ...
    if(canal && (canal.type == ChannelType.GuildText || canal.type == ChannelType.GuildAnnouncement)){
      canal.send({embeds: [embEncuesta], content: `**¡Nueva encuesta <@&${dataEnc?.data.rolId}>!**`}).then(async ts=>{
        ...
        for(let r=0; r<opciones.length; r++){
          if(dataEnc) ts.react(dataEnc.data.emojis[r])
        }
        int.reply({ephemeral: true, embeds: [embEnviado]})
        arrayEn?.push({id: ts.id, channelId: canal.id, ends: Math.floor(Date.now()+ms(tiempo)), authorId: int.user.id, createdAt: Date.now(), active: true, options: opciones})
  ...
  if(subCommand == "sorteo"){
    let dataSor = await rafflesModel.findById(serverId), arraySo = dataSor?.raffles, colorEb = guild?.roles.cache.get(dataSor?.data.rolId || '')?.hexColor
    ...
    if(canal && (canal.type == ChannelType.GuildText || canal.type == ChannelType.GuildAnnouncement)){
      canal.send({embeds: [embSorteo], content: `**¡Nuevo sorteo <@&${dataSor?.data.rolId}>**!`}).then(async ts=>{
        ...
        int.reply({ephemeral: true, embeds: [embEnviado]})
        arraySo?.push({id: ts.id, channelId: canal.id, ends: Math.floor(Date.now()+ms(tiempo)), winners: ganadores || 1, authorId: int.user.id, createdAt: Date.now(), active: true, participants: []})
