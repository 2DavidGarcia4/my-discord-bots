...
import { botModel, carcelModel } from "../../../models";
import { sendMessageSlash, setErrors, setSlashError, setSlashErrors } from "../../../utils/functions";
...
export const encarcelarSlashCommand = async (int: ChatInputCommandInteraction<CacheType>, client: Client) => {
  ...
  member?.roles.add("830260549098405935").then(async c=>{
    ...
    await carcelModel.findByIdAndUpdate(client.user?.id, {cantidad, prisioneros: pricioners})
  })
