...
exports.timeoutScb = new discord_js_1.SlashCommandBuilder()
    ...
    .toJSON();
const timeoutSlashCommand = async (int, client) => {
    ...
    member?.timeout((0, ms_1.default)(time), timeoutReazon).then(() => {
        member?.send({ embeds: [WarnDMEb], files: image ? [image] : [] }).catch(() => {
            if (WarnEb.data.footer)
                WarnEb.data.footer.text = isEnglish ? `I could not send the message to the former member ${member.user.tag}` : `No he podido enviar el mensaje al ex miembro ${member?.user.tag}`;
        }).finally(() => {
